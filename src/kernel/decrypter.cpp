...
extern "C"
{
#include "../../vendors/libkirk/kirk_engine.h"
#include "../../vendors/libkirk/SHA1.h"
}

#define ROUNDUP16(x)  (((x)+15)&~15)


// Stolen from ppsspp and pspdecrypt
...
{
	spdlog::info("Decrypting tag {:02X}", *(u32*)&inbuf[0xD0]);
	const auto decryptSize = *(int*)&inbuf[0xB0];
	const auto pti = GetTagInfo(*(u32*)&inbuf[0xD0]);
...
static int pspDecryptType2(const u8 *inbuf, u8 *outbuf, u32 size)
{
	spdlog::info("Decrypting tag {:02X}", *(u32*)&inbuf[0xD0]);
	const auto decryptSize = *(int*)&inbuf[0xB0];
	const auto pti = GetTagInfo2(*(u32 *)&inbuf[0xD0]);
